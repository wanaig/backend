...
**接口**: `POST /member/wxlogin`

**是否需要登录**: 否

**接口说明**: 微信一键登录/注册接口，只需code即可完成登录或注册。如果用户不存在，自动创建新用户。

**登录流程**:
```
wx.login() → 获取 code
    ↓
POST /member/wxlogin { code: "xxx" }
    ↓
后端自动：用 code 换 openid → 查询/创建会员 → 返回 token
```
...
async onWxLogin() {
  ...
  const res = await wx.request({
    url: 'http://localhost:8080/member/wxlogin',
    method: 'POST',
    data: { code }
  })
  ...
  if (res.data.code === 0) {
    ...
    this.$store.commit('SET_MEMBER', loginData)
  }
}
```
